# app/routes/prophecies_tgp.py
# Full path: MyVineChurch/app/routes/prophecies_tgp.py
# File name: prophecies_tgp.py
# Brief, detailed purpose: Complete, clean blueprint for Prophecies module.
# Single URL /prophecies_tgp for listing (public for guests, all for logged-in).
# Full support for public/private/personal visibility.
# Submit / edit / delete + comment system with censorship and audit logging.
# All templates and routes now match your existing prophecies_tgp.html exactly.
...
prophecies_bp = Blueprint('prophecies_tgp', __name__, url_prefix='/prophecies_tgp')

REQUIRED_ROLES = ['Admin', 'Owner']


# ----------------------------------------------------------------------
# Main Listing – /prophecies_tgp (single URL)
# ----------------------------------------------------------------------
@prophecies_bp.route('/')
def list_prophecies():
    ...
    return render_template('prophecies_tgp/prophecies_tgp.html',
...
# ----------------------------------------------------------------------
# Single Prophecy Detail View – /prophecies_tgp/<int:prophecy_id>
# ----------------------------------------------------------------------
...
def view_prophecy(prophecy_id):
    ...
    if not prophecy:
        ...
        return redirect(url_for('prophecies_tgp.list_prophecies'))
    ...
    return render_template('prophecies_tgp/view_prophecy.html',
                           prophecy=prophecy,
                           comments=comments,
                           is_logged_in=is_logged_in,
                           can_edit=can_edit,
...
# ----------------------------------------------------------------------
# Add Prophecy – /prophecies_tgp/add
# ----------------------------------------------------------------------
...
def add_prophecy():
    if request.method == 'POST':
        ...
        if not title or not description:
            ...
            return redirect(url_for('prophecies_tgp.add_prophecy'))
        ...
        try:
            cur.execute("""
                INSERT INTO prophecies_tgp (title, description, visibility, user_id, created_at, updated_at)
    ...
    return render_template('prophecies_tgp/add_prophecy.html')


# ----------------------------------------------------------------------
# Edit Prophecy – /prophecies_tgp/edit/<int:prophecy_id>
# ----------------------------------------------------------------------
...
def edit_prophecy(prophecy_id):
    db = get_db()
    cur = db.cursor(pymysql.cursors.DictCursor)
    cur.execute("SELECT * FROM prophecies_tgp WHERE id = %s", (prophecy_id,))
    prophecy = cur.fetchone()
    ...
    if request.method == 'POST':
        ...
        if not title or not description:
            flash('Title and description are required.', 'error')
            return redirect(url_for('prophecies_tgp.edit_prophecy', prophecy_id=prophecy_id))
    ...
    return render_template('prophecies_tgp/edit_prophecy.html', prophecy=prophecy)


# ----------------------------------------------------------------------
# Delete Prophecy – /prophecies_tgp/delete/<int:prophecy_id>
# ----------------------------------------------------------------------
...
def delete_prophecy(prophecy_id):
    ...
    cur = db.cursor()
    try:
        cur.execute("DELETE FROM prophecies_tgp WHERE id = %s", (prophecy_id,))
        db.commit()
        if cur.rowcount:
...
# ----------------------------------------------------------------------
# Add Comment – /prophecies_tgp/comment/add/<int:prophecy_id>
# ----------------------------------------------------------------------
...
def add_comment(prophecy_id):
    ...
    if not comment_text:
        ...
        return redirect(url_for('prophecies_tgp.view_prophecy', prophecy_id=prophecy_id))
...
def edit_comment(comment_id):
    ...
    return redirect(url_for('prophecies_tgp.view_prophecy', prophecy_id=comment['prophecy_id']))
